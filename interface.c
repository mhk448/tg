...
void do_load_user_photo2  (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
  ...
  tgl_peer_t *P = tgl_peer_get_by_name (TLS, args[n].str); 
...
struct command commands[MAX_COMMANDS_SIZE] = {
  ...
  {"load_user_photo2", {ca_string, ca_none}, do_load_user_photo2, "load_user_photo2 <user>\tDownloads file to downloads dirs2. Prints file name after download end2", NULL},
